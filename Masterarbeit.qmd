...
```
```{r}
ggplot(night_avg_5, aes(x = Minute5_Factor, y = Avg_Glucose_5, color = ID)) +
  geom_smooth(aes(fill = ID), size = 0.7, se = TRUE, alpha = 0.2) +
  geom_line(size = 0.5) +
  scale_x_discrete(breaks = hour_labels) +
  facet_wrap(~ ID) +
  labs(
...
ggplot(synthetic_24h, aes(x = datetime, y = glucose)) +
  ...
  labs(
    title = "Synthetic Glucose Curve (Min at 6:00 , Max bei 18:00)",
...
# --- Pakete -----------------------------------------------------------------
pkgs <- c("dplyr","lubridate","ggrepel","purrr","tibble","readr","lomb","stringr","ggplot2")
...
```{r}
# --- Parameter ---------------------------------------------------------------
OFAC           <- 4          # gewünschte Oversampling-Faktor
ROUND_MINUTES  <- FALSE      # TRUE = Perioden runden; FALSE = hohe Auflösung
MIN_POINTS     <- 60         # Mindestanzahl Messpunkte pro Nacht
MAX_PERIOD_MIN <- 180        # maximale Periode in Minuten

# --- Korrigierte Lomb-Funktion (Perioden-Einheit = Minuten) ------------------
lomb_for_span <- function(df, max_period_min = 180, ofac = 4,
                          detrend = TRUE, round_minutes = FALSE) {
  # Glukose-Spalte vereinheitlichen
  ...
  # Zeitachse in MINUTEN seit Start
  t_min <- as.numeric(df$Timestamp - df$Timestamp[1], units = "mins")
  
  # optional detrend: entfernt Offset + linearen Trend
  x <- df$glucose
  if (detrend) {
    x <- resid(lm(x ~ t_min))
  } else {
  ...
  # Sampling-Schritt und sichere Untergrenze
  dt_min <- stats::median(diff(t_min), na.rm = TRUE)
  min_period_min <- max(2 * dt_min, 10)  # Nyquist-sicher, mind. 10 min
  
  # WICHTIG: type="period" → from/to sind PERIODEN (Minuten), passend zu times=t_min
  ls <- lomb::lsp(
    x     = x,
    times = t_min,
    from  = min_period_min,     # Periode (min)
    to    = max_period_min,     # Periode (min)
    type  = "period",
    ofac  = ofac,
    plot  = FALSE
  )
  
  period_vec <- as.numeric(ls$scanned)  # bereits Minuten
...
# --- Daten vorbereiten: nächtliche Segmente (22–06) --------------------------
...
# Nächte mit genügend Punkten behalten
dat_nights <- dat %>%
  dplyr::group_by(ID, night_date) %>%
  dplyr::filter(dplyr::n() >= MIN_POINTS) %>%
  dplyr::ungroup()

# --- TEST: 1 ID × 2 Nächte (optional) ---------------------------------------
test_dat <- dat_nights %>%
 ...
 # --- Pooling-Variante 1: "jede Nacht zählt gleich" --------------------------
 ...
 # --- Beispielplot für pooled_by_night ---------------------------------------
 ...
 ggplot(plot_df, aes(period_minutes, mean_power)) +
   ...
   labs(title = "Pooled Lomb–Scargle Spectrum (per night)",
        subtitle = "jede Nacht gleich gewichtet · ofac = 4",
        x = "Periode (Minuten)", y = "Mean Power") +
   theme_minimal()
 
 
 # --- Per-ID summaries (already stored in your .rds) --------------------------
...
```{r, warning=FALSE}
# =========================================
# GESAMT: Wavelet-Power (Morlet) + GWS, gepoolt über alle IDs × Nächte
# =========================================

# ------------------ Pakete & Parameter ------------------
pkgs <- c("dplyr","lubridate","ggplot2","tibble","purrr","zoo","biwavelet",
          "tidyr","stringr","gridExtra","viridis")
to_install <- pkgs[!sapply(pkgs, requireNamespace, quietly = TRUE)]
if (length(to_install)) install.packages(to_install)
invisible(lapply(pkgs, library, character.only = TRUE))

PER_MIN <- 10     # min. Periode (Minuten)
PER_MAX <- 180    # max. Periode (Minuten)
DJ      <- 1/12   # Skalenauflösung (kleiner = feiner)
STEP    <- 5      # Zielraster Minuten
MINPTS  <- 60     # mind. Messpunkte/Nacht (≈5h bei 5-min Raster)

# ------------------ Hilfsfunktionen ------------------
# 1) Nacht vorbereiten: 5-min Raster, Interpolation, t_min, detrend + center
...
# 2) Wavelet (biwavelet::wt), Perioden in Minuten
...
# 3) GWS (Global Wavelet Spectrum) innerhalb COI
...
# 4) Peak-Finder (auf geglätteter Kurve)
...
# ------------------ Aggregation über alle IDs × Nächte ------------------
...
# ------------------ GEPOOLTE HEATMAP (links) + GWS (rechts) ------------------
# 1) Alle Nächte erneut als Wavelet → pooled Heatmap innerhalb COI
wt_list <- list()
...
p_left <- ggplot(df_pool, aes(time, period, fill = power)) +
  ...
  labs(title = "Wavelet Power (Morlet) – gepoolt (alle IDs × Nächte)",
       subtitle = "grau = außerhalb der COI (unsicher)",
       x = "Minuten seit Nachtbeginn", y = "Periode (Minuten)") +
  theme_minimal()

# 2) Global Wavelet Spectrum (rechts) – geglättet + Peaks
...
p_right <- ggplot(plot_all, aes(period, mean_power)) +
  ...
  labs(title = "Global Wavelet Spectrum – pooled (alle IDs × Nächte)",
       subtitle = "jede Nacht gleich gewichtet · nur innerhalb COI",
       x = "Periode (Minuten)", y = "Mean Power") +
  theme_minimal()

# 3) zusammen anzeigen & speichern
...
```
```{r}
# ------------------ Extra Plot: GWS pro ID ------------------
p_per_id <- ggplot(all_gws_pooled, aes(period, mean_power, color = ID)) +
  geom_line(alpha = 0.9) +
  scale_x_continuous(breaks = seq(PER_MIN, PER_MAX, by = 10)) +
  labs(title = "Global Wavelet Spectrum – pro ID (über Nächte gemittelt)",
       subtitle = "jede Nacht gleich gewichtet",
       x = "Periode (Minuten)", y = "Mean Power") +
  theme_minimal()
