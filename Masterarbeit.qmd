...
ggplot(night_avg, aes(x = Hour_Factor, y = Avg_Glucose, group = ID, color = ID)) +
  ...
  labs(
    ...
    subtitle = "Hours from 22:00 to 06:00: hourly bins",
    x = "Hour of Night",
...
```
### Manual Consinus fit but with datasets prepared for fit with cosinor() function (-> no hourly bins but all datapoints)
...
df_112_first14 <- cosinor_daily_predictions %>%
  filter(ID == "112") %>%
  mutate(date = as.Date(datetime)) %>%
  filter(date %in% sort(unique(date))[14:28])  # keep only second 14 days
...
for (id in ids) {
  ...
  while (current_start + window_length <= end_time) {
    ...
    # Kosinus-Komponenten neu berechnen (zur Sicherheit)
...
```
**Description:** Add Seasons to Moving Window Data Frames
```{r}
...
moving_window_107 <- moving_window_107 %>%
  mutate(Season = sapply(Window_Start, seasons))

moving_window_108 <- moving_window_108 %>%
  mutate(Season = sapply(Window_Start, seasons))

moving_window_109 <- moving_window_109 %>%
  mutate(Season = sapply(Window_Start, seasons))

moving_window_112 <- moving_window_112 %>%
  mutate(Season = sapply(Window_Start, seasons))
```
**Description:** Calculate Amplitude, Acrophase and Intercept Means per Season
```{r}
 amplitude_means_102 <- moving_window_102 %>%
    ...
    summarise(
      ...
      mean_acr = mean(Acrophase_Raw_Hours, na.rm = TRUE),
      sd_acr = sd(Acrophase_Raw_Hours, na.rm = TRUE),
      n = n()
    )

amplitude_means_104 <- moving_window_104 %>%
    group_by(Season) %>%
    summarise(
      mean_amp = mean(Amp, na.rm = TRUE),
      sd_amp = sd(Amp, na.rm = TRUE),
...
amplitude_means_107 <- moving_window_107 %>%
    group_by(Season) %>%
    summarise(
      mean_amp = mean(Amp, na.rm = TRUE),
      sd_amp = sd(Amp, na.rm = TRUE),
...
amplitude_means_108 <- moving_window_108 %>%
    group_by(Season) %>%
    summarise(
      mean_amp = mean(Amp, na.rm = TRUE),
      sd_amp = sd(Amp, na.rm = TRUE),
...
amplitude_means_109 <- moving_window_109 %>%
    group_by(Season) %>%
    summarise(
      mean_amp = mean(Amp, na.rm = TRUE),
      sd_amp = sd(Amp, na.rm = TRUE),
...
amplitude_means_112 <- moving_window_112 %>%
    group_by(Season) %>%
    summarise(
      mean_amp = mean(Amp, na.rm = TRUE),
      sd_amp = sd(Amp, na.rm = TRUE),
...
```{r}
  ...
  model_acr <- lmer(mean_acr ~ Season + (1 | ID), data = means_cos_fit)
...
**Description:** Plot
```{r}
  ggplot(means_cos_fit, aes(x = Season, y = mean_acr)) +
    geom_boxplot(fill = "lightblue", alpha = 0.6) +
    geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
    theme_minimal() +
    labs(title = "Mean Acrophase by Season",
         x = "Season",
         y = "Mean Acrophase") +
...
```{r}
  # Model: Mean amplitude by Season with random intercept for ID
  model_int <- lmer(mean_int ~ Season + (1 | ID), data = means_cos_fit)
...
**Description:** Plot
```{r}
  ggplot(means_cos_fit, aes(x = Season, y = mean_int)) +
    geom_boxplot(fill = "lightblue", alpha = 0.6) +
    geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
    theme_minimal() +
    labs(title = "Mean Intercept by Season",
         x = "Season",
         y = "Mean Intercept") +
    theme(plot.title = element_text(hjust = 0.5))
```

### Seasonal Cosinus Fit: Raw Data
...
### Polar Plots 
#### Polar Plot for Max and Min for ID 102
**Description:** Code for Polar Plot (ID 102)
```{r}
...
``` 
**Description:** Create Polar Plot for 102
```{r}
  ggplot() +
...
```
#### Polar Plot for Max for all IDs
